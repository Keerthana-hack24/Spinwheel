...
// === Backend config & helpers (ADD THIS) ===
const API_BASE = "http://127.0.0.1:5050";

// Tell the backend who won (so it can set status='winner' in the table)
async function markWinnerInBackend(name) {
  try {
    await fetch(`${API_BASE}/winner`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ name })
    });
  } catch (err) {
    console.log("Failed to mark winner in backend:", err);
  }
}
...
// Pull names from the database on page load and redraw the wheel (ADD THIS AT END)
window.addEventListener('load', async () => {
  try {
    ...
    console.log('Could not fetch names from backend; keeping defaults.', e);
  }
});
